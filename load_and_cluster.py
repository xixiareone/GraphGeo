...
def get_idx(num, seed, lm_ratio):
    idx = list(range(0, num))
    random.seed(seed)
    random.shuffle(idx)
    lm_train_num = int(num * lm_ratio * lm_ratio)
    tg_train_num = int(num * lm_ratio * (1 - lm_ratio))
    lm_train_idx, tg_train_idx, tg_test_idx = idx[:lm_train_num], idx[lm_train_num:tg_train_num + lm_train_num], idx[
                                                                                                                 lm_train_num + tg_train_num:]
...
if __name__ == '__main__':
    # dataset = "Los_Angeles"  # Shanghai/New_York/Los_Angeles
    seed = opt.seed
    lm_ratio = opt.lm_ratio
    lm_train_idx, tg_train_idx, lm_test_idx, tg_test_idx = get_idx(len(get_XY(opt.dataset)[0]), seed, lm_ratio)
